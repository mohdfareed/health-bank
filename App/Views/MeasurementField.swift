...
struct MeasurementField<D: Dimension>: View {
    ...
    init(
        ...
        self._inputValue = State(initialValue: self.$measurement.wrappedValue.value)
    }
...
extension MeasurementField {
    private func valueField() -> some View {
        ...
        .foregroundStyle(validationError == nil ? .primary : .secondary)
        ...
        .onChange(of: $measurement.overrideUnit) { oldValue, newValue in
            if newValue == nil && oldValue != nil {  // Reset was chosen via the nil tag
                if let computedValue = vm.computed {
                    ...
                    let computedMeasurementInBase = Measurement(
                        value: computedValue, unit: $measurement.definition.baseUnit)
                    // If overrideUnit becomes nil, effectiveUnit in LocalizedMeasurement will be locale-based
                    let targetUnitForDisplay = $measurement.definition.unit(
                        for: $measurement.locale)
                    let computedInTargetUnit = computedMeasurementInBase.converted(
                        to: targetUnitForDisplay)
    ...
    private func computedValueText() -> Text? {
        ...
        let computedInEffectiveUnit = Measurement(
            value: computedValue, unit: $measurement.definition.baseUnit
        )
        ...
        return Text(Image(systemName: "function"))
            + Text(" Est: \(formattedValue) \(effectiveUnit.symbol)")
    }
